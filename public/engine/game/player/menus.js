...
(function (root, factory) {
    ...
    class Menu {
        ...
        step() {
            // Only process menu interactions if there are buttons
            if (this.buttons.length === 0) return;

            let x, y;
            // if the player's controller is keyboard
            if (game.player && game.player.controller) {
                ...
                for (const button of this.buttons) {
                    ...
                    if (
                        ...
                        if (game.player.controller.buttons.fire.current || game.player.controller.buttons.inventory2.current || game.player.controller.type == 'touch') {
    ...
    */
    class Menu_Pause extends Menu {
        constructor(buttons, shape, options) {
            ...
            this.type = 'pause';
            this.buttons = [
                new Menu_Button(new Utils.Rect(0, 0, 150, 30), "Restart Match", function () { game.match.reset(); game.match.setup(); game.paused = false; }),
                new Menu_Button(new Utils.Rect(0, 40, 150, 30), "Quit to Menu", function () { game.match = new Start_Screen(); game.paused = false; game.menu = game.menus.main }),
                // new Menu_Button(new Utils.Rect(0, 80, 150, 30), "Button 3", function () { window.alert("Button 3"); }),
                new Menu_Button(new Utils.Rect(0, 120, 150, 30), "Resume", function () { game.paused = false; })
            ]
        }
    }
    ...
    */
    class Menu_AwaitingInput extends Menu {
        constructor(buttons, shape, options) {
            super(buttons, shape, options);
            this.style = 'center_stacked';
            this.type = 'awaiting';
    ...
    */
    class Menu_AwaitingPlayers extends Menu {
        constructor(buttons, shape, options) {
            super(buttons, shape, options);
            this.style = 'center_stacked';
            this.type = 'awaiting';
            this.title = 'Waiting for all players';
            this.buttons = []
        }
        draw() {
            if (this.visible == false) return;
            ...
            ctx.font = '20px Jura';
            ctx.textAlign = "center";
            ctx.fillText(this.title, offsetx + this.shape.w / 2, offsety + this.shape.h / 2 - 10);

            // draw player count
            if (game && game.match && game.match.playerLimit) {
                let currentPlayers = game.countConnections ? game.countConnections() : (game.players ? game.players.length : 0);
                let maxPlayers = game.match.playerLimit.max;
                ctx.font = '16px Jura';
                ctx.fillText(`${currentPlayers} / ${maxPlayers} players`, offsetx + this.shape.w / 2, offsety + this.shape.h / 2 + 20);
            }
        }
    }
    ...
    */
    class Menu_Main extends Menu {
        constructor(buttons, shape, options) {
            ...
            this.type = 'pause';
            this.buttons = [
                new Menu_Button(new Utils.Rect(0, 0, 150, 30), "For Speed", function () { game.match = new Match_ForSpeed(); game.menu = null; game.paused = false; }),
                new Menu_Button(new Utils.Rect(0, 40, 150, 30), "For Honor", function () { game.match = new Match_ForHonor(); game.menu = null; game.paused = false; }),
                new Menu_Button(new Utils.Rect(0, 80, 150, 30), "For Ever", function () { game.match = new Match_ForEver(); game.menu = null; game.paused = false; }),
                new Menu_Button(new Utils.Rect(0, 120, 150, 30), "debugmode", function () { game.match = new DebugMatch(); game.menu = null; game.paused = false; }),
                new Menu_Button(new Utils.Rect(0, 160, 150, 30), "Exit", function () { window.close(); })
            ]
        }
    }
